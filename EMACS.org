...
(use-package meow
  ...
  (defvar my-eglot-map
    (let ((map (make-sparse-keymap)))
      ...
      (define-key map (kbd "a") #'eglot-format-buffer)
...
(use-package eglot
  :ensure nil
  :hook ((rust-ts-mode bash-ts-mode nix-mode) . eglot-ensure)
...
(use-package prism
  ...
  :config (setq prism-comments nil))
...
(use-package emacs
  ...
  (setq whitespace-style '(face tabs trailing space space-mark indentation lines empty))
